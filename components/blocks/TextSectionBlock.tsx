...
export function TextSectionBlock({ value }: { value: TextSectionBlockValue }) {
  return (
    <section className="mx-auto max-w-4xl px-6 py-12">
      {value.eyebrow ? (
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-600">{value.eyebrow}</p>
      ) : null}
      {value.title ? <h2 className="mt-2 text-3xl font-semibold text-slate-950">{value.title}</h2> : null}
      {value.body ? (
        <div className="mt-4">
          <PortableText
